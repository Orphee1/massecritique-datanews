...
export default function VXLinepath({ data }) {
  ...
  const handleTooltip = ({ event, data, xStock, xScale, yScale }) => {
    ...
    if (d12 && d12.date) {
      d2 = x0 - xStock(d02.date) > xStock(d12.date) - x0 ? d12 : d02;
    }
    if (d13 && d13.date) {
      d3 = x0 - xStock(d03.date) > xStock(d13.date) - x0 ? d13 : d03;
    }
    if (d14 && d14.date) {
      d4 = x0 - xStock(d04.date) > xStock(d14.date) - x0 ? d14 : d04;
    }
    showTooltip({
      ...
      {tooltipData && (
        <div>
          ...
          >
            ...
            >
              Hospitalisations: <strong>{yStock(tooltipData[1])}</strong>
              <br />
              Décès: <strong>{yStock(tooltipData[0])}</strong>
              <br />
              Réanimations: <strong>{yStock(tooltipData[2])}</strong>
              <br />
              Retours à domicile: <strong>{yStock(tooltipData[3])}</strong>
            </div>
          </Tooltip>
          <Tooltip
            top={yMax + 90}
